...
function install(root) {
    ...
    if (fs.existsSync(appDir)) {
        if (!fs.existsSync(dst)) {
            ...
            console.log('Hook was successfully installed');
        } else {
            ...
            if (!/function stss/.test(content)) {
                ...
                try {
                    ...
                    if (installedVersionNr >= newVersionNr) {
                        console.log('Hook was already installed');
                    } else {
                        newSource = newSource.match(reFunction)[0];
                        content = content.replace(reFunction, newSource);
                        fs.writeFileSync(dst, content);
                        console.log('Hook was successfully updated to v'+newVersion.join('.'));
                    }
                } catch(e) {
                    console.log('ERROR: Hook could not be updated to v'+newVersion.join('.') + ' automatically, please update it manually');
                }
            }
        }
    } else {
